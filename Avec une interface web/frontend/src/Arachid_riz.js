...
function Arachid_riz() {
    ...
    const chargerDonnees = async () => {
        try {
            const reponse = await fetch('http://127.0.0.1:8000/arachides_riz/donneInitiale');
            if (reponse.ok) {
    ...
    const chargerDonneesGenere = async (event) => {
        ...
        try {
            const reponse = await fetch('http://127.0.0.1:8000/arachides_riz/donneGenere');
            if (reponse.ok) {
                const texte = await reponse.text();
                const lignes = texte.split('\n');
                const entetes = lignes[0].split(',');
    ...
    const handlelancelagenreation = async () => {
        try {
            const response = await fetch('http://127.0.0.1:8000/arachides_riz/genere', {
                method: 'POST',
                headers: {
    ...
    <div>
       <div className="flex items-center my-8 pl-8">
               <p className="text-4xl font-medium mt-5 leading-none text-teal-500 mr-2">Augmentation de données pour le fichier d'arachides et riz'</p>
            </div> 
                        ...
                        <div className="overflow-hidden">
                            <table className="w-full text-center text-sm font-light">
                                <thead className="border-b bg-teal-500 font-medium text-white">
                                    ...
                                    <th scope="col" className="px-6 py-4">CropType</th>
                                    <th scope="col" className="px-6 py-4">CropDays</th>
                                    <th scope="col" className="px-6 py-4">Soil Moisture (%)</th>
                                    <th scope="col" className="px-6 py-4">Soil Temperature (°C)</th>
                                    <th scope="col" className="px-6 py-4">Temperature (°C)</th>
                                    <th scope="col" className="px-6 py-4">Humidity (%)</th>
                                    <th scope="col" className="px-6 py-4">Irrigation(Y/N)</th>
                                    </tr>
                                </thead>
                                <tbody>
      ...
      <div className="flex w-full  m-4 flex-col">
            ...
            {QualiteDesDonneesGenere &&  (
                 ...
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    ...
                    <div> 
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/arachides_rizcumulative_${paramtre}.png`} alt="Arachide et riz cumulative" />
                    </div>
                 </div>
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center my-4 pl-4">
                    ...
                    <div> arachides_rizboxplot_Humidity
                    <img src={`${process.env.PUBLIC_URL}/Image_genere/arachides_rizboxplot_${paramtre}.png`} alt="Arachide et riz boxplot" />

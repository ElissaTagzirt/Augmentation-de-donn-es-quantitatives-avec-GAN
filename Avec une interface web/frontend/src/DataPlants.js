...
function DataPlants() {
    ...
    const chargerDonnees = async () => {
        try {
            const reponse = await fetch('http://127.0.0.1:8000/dataPalntes/donneInitiale');
            if (reponse.ok) {
    ...
    const chargerDonneesGenere = async (event) => {
        ...
        try {
            const reponse = await fetch('http://127.0.0.1:8000/potato/donneGenere');
            if (reponse.ok) {
                const texte = await reponse.text();
                const lignes = texte.split('\n');
                const entetes = lignes[0].split(',');
    ...
    const handlelancelagenreation = async () => {
        try {
            const response = await fetch('http://127.0.0.1:8000/dataPalntes/genere', {
                method: 'POST',
                headers: {
    ...
    <div>
       <div className="flex items-center my-8 pl-8">
               <p className="text-4xl font-medium mt-5 leading-none text-teal-500 mr-2">Augmentation de données pour le fichier Data Plants</p>
            </div> 
          ...
          <div className="overflow-x-auto mt-4">
              <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                  <div className="overflow-hidden">
                      <table className="w-full text-center text-sm font-light">
                          <thead className="border-b bg-teal-500 font-medium text-white">
                              ...
                              <th scope="col" className="px-6 py-4">Water Req en mm</th>
                              <th scope="col" className="px-6 py-4">Month</th>
                              <th scope="col" className="px-6 py-4">Min Temp en °C</th>
                              <th scope="col" className="px-6 py-4">Max Temp en °C</th>
                              <th scope="col" className="px-6 py-4">Humidity </th>
                              <th scope="col" className="px-6 py-4">Wind</th>
                              <th scope="col" className="px-6 py-4">Sun</th>
                              <th scope="col" className="px-6 py-4">Rad</th>
                              <th scope="col" className="px-6 py-4">Rain</th>
                              <th scope="col" className="px-6 py-4">Altitude</th>
                              <th scope="col" className="px-6 py-4">Latitude</th>
                              <th scope="col" className="px-6 py-4">Longitude</th>
                  ...
                  <div className="flex items-center my-8 pl-4">
                    ...
                    Le nombre de lignes de données non réellement générées   <span className='text-lg font-medium m-5 leading-none text-teal-500'>{Nbligne}</span>
                       </p>
                    </div>
      ...
      <div className="flex w-full  m-4 flex-col">
            ...
            {QualiteDesDonneesGenere &&  (
                 ...
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    ...
                    <div>
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/plantshisto_month_${paramtre}.png`} alt="Plantes Histogramme" />
                    </div>
                 </div>
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center my-4 pl-4">
                    ...
                    <div> 
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/plantscumulative_${paramtre}.png`} alt="Plantes cumulative" />
                    </div>
                 </div>
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center my-4 pl-4">
                    ...
                    <div> 
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/plantsboxplot_${paramtre}.png`} alt="Plantes boxplot" />
                    </div>
                 </div>
             </div>
            )}

...
function Riz() {
    ...
    const chargerDonnees = async () => {
        try {
            const reponse = await fetch('http://127.0.0.1:8000/riz/donneInitiale');
            if (reponse.ok) {
    ...
    const chargerDonneesGenere = async (event) => {
        ...
        try {
            const reponse = await fetch('http://127.0.0.1:8000/riz/donneGenere');
            if (reponse.ok) {
                const texte = await reponse.text();
                const lignes = texte.split('\n');
                const entetes = lignes[0].split(',');
    ...
    const handlelancelagenreation = async () => {
        try {
            const response = await fetch('http://127.0.0.1:8000/riz/genere', {
                method: 'POST',
                headers: {
    ...
    <div>
       <div className="flex items-center my-8 pl-8">
               <p className="text-4xl font-medium mt-5 leading-none text-teal-500 mr-2">Augmentation de données pour le fichier Riz</p>
            </div> 
...
<form>*
            ...
            {QualiteDesDonneesGenere &&  (
                 ...
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    ...
                    <div>
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/rizhisto_month_${paramtre}.png`} alt="Riz boxplot" />
                    </div>
                 </div>
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center my-4 pl-4">
                    ...
                    <div> 
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/rizcumulative_${paramtre}.png`} alt="Riz boxplot" />
                    </div>
                 </div>
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center my-4 pl-4">
                    ...
                    <div> 
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/rizboxplot_${paramtre}.png`} alt="Riz boxplot" />
                    </div>
                 </div>
             </div>
            )}

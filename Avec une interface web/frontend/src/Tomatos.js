...
function Tomatos() {
    ...
    const [QualiteDesDonneesGenere, setQualiteDesDonneesGenere]=useState(true);
    ...
    const chargerDonnees = async () => {
        try {
            const reponse = await fetch('http://127.0.0.1:8000/tomates/donneInitiale');
            if (reponse.ok) {
    ...
    const chargerDonneesGenere = async (event) => {
        ...
        try {
            const reponse = await fetch('http://127.0.0.1:8000/tomates/donneGenere');
            if (reponse.ok) {
                const texte = await reponse.text();
                const lignes = texte.split('\n');
                const entetes = lignes[0].split(',');
    ...
    const handlelancelagenreation = async () => {
        try {
            const response = await fetch('http://127.0.0.1:8000/tomates/genere', {
                method: 'POST',
                headers: {
    ...
    <div>
       <div className="flex items-center my-8 pl-8">
               <p className="text-4xl font-medium mt-5 leading-none text-teal-500 mr-2">Augmentation de données pour le fichier Tomates</p>
            </div> 
                        ...
                        <div className="overflow-hidden">
                            <table className="w-full text-center text-sm font-light">
                                <thead className="border-b bg-teal-500 font-medium text-white">
                                    ...
                                    <th scope="col" className="px-6 py-4">Simulation ID</th>
                                    <th scope="col" className="px-6 py-4">Time (s)</th>
                                    <th scope="col" className="px-6 py-4">Water (L/m²)</th>
                                    <th scope="col" className="px-6 py-4">Hour (h)</th>
                                    </tr>
                                </thead>
                                <tbody>
      ...
      <div className="flex w-full  m-4 flex-col">
            ...
            {QualiteDesDonneesGenere &&  (
                 ...
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    ...
                    <div>
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/tomatoshisto_month_${paramtre}.png`} alt="Tomates Histogramme" />
                    </div>
                 </div>
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center my-4 pl-4">
                    ...
                    <div> 
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/tomatoscumulative_${paramtre}.png`} alt="Tomates cumulative" />
                    </div>
                 </div>
                 <div className="inline-block min-w-full py-2 px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center my-4 pl-4">
                    ...
                    <div> 
                        <img src={`${process.env.PUBLIC_URL}/Image_genere/tomatosboxplot_${paramtre}.png`} alt="Tomates boxplot" />
                    </div>
                 </div>
             </div>
            )}

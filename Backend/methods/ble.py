...
def after_generation(generated_df, city_cols, month_cols, chemin_sortie, months_list):
    # Ajouter la colonne 'Crop' avec la valeur 'blé' partout
    generated_df['Crop'] = 'blé'
    
    # Ajouter la colonne 'Soil' avec la valeur 'red sandy' partout
    generated_df['Soil'] = 'red sandy'
    ...
    df_base, df_generated = charger_Fichier("csv", 'data/ble.csv', 'data_genere/ble_generated.csv')
    params = ['water req', 'Min Temp', 'Max Temp', 'Humidity','Wind','Sun','Rad','rain','altitude','latitude']
    '''for param in params:
         representer_selon_mois_histogramme(param, 'ble', df_base, df_generated)
         representer_selon_mois_boxplot(param, 'ble', df_base, df_generated)
         plot_cumulative_frequency(param, 'ble', df_base, df_generated)'''
...
def ble_train(chemin_lecture, latent_dim, epochs, batch_size, sample_interval, culture):
    cat_variables = ['city', 'month']
    COLS_USED = ['water req','month','Min Temp','Max Temp','Humidity','Wind','Sun','Rad','rain','altitude','latitude','city']
...
def ble_generator(chemin_sortie, num_samples, culture):
    city_cols = ['city_chennai','city_delhi','city_jaipur','city_kolkata','city_kurnool','city_mumbai']
    month_cols = ['month_April','month_July','month_June','month_March','month_May']
    months_list = ['April','July','June','March','May']

...
def after_generation(generated_df, city_cols, month_cols, chemin_sortie, months_list):
    # Ajout des colonnes 'Soil' et 'Crop' avec les valeurs spécifiées
    generated_df['Soil'] = 'red loamy'
    generated_df['Crop'] = 'potato'
...
def potato_train(chemin_lecture, latent_dim, epochs, batch_size, sample_interval, culture):
...
def potato_generator(chemin_sortie, num_samples, culture):
...
potato_train('./data/potato.csv', 100, train_nb, 32, 100, 'potato')

# Géneration des données
potato_generator('./data/potato_generated.csv', 40, 'potato')
    
# Qualité des données
df_base, df_generated = charger_Fichier("csv", './data/potato.csv', './data/potato_generated.csv')

# Histogramme
representer_selon_mois_histogramme('water req', 'potato', df_base, df_generated)

# Boxplot
representer_boxplot('water req', 'potato', df_base, df_generated)

# Fréquences cumulées
plot_cumulative_frequency('water req', 'potato', df_base, df_generated)
